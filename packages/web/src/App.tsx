...
export function Dashboard({ initialTab }: { initialTab?: string } = {}) {
  ...
  const actions = useMemo(() => {
    ...
    const claudeMdLines: string[] = [
      ...
      '',
      '## Pre-Action Checkpoint (mandatory — runs before EVERY implementation attempt)',
      'Before writing ANY code or running ANY command to fix/solve a problem, answer these two questions out loud:',
      '1. **Attempt count:** Is this my 2nd+ attempt at this specific problem? → STOP. Suggest /model opus. Write nothing else. Do not "just try one more thing."',
      '2. **Model fit:** Am I on the right model for this task? → State the model decision before proceeding, even if the answer is "stay on current model."',
      '',
      'Skipping this checkpoint to "be helpful" or "keep momentum" is a rule violation, not helpfulness. The cost of pausing is zero. The cost of burning attempts on the wrong model is real.',
      '',
    ...
    if (canUseOpusPlan) {
      ...
      claudeMdLines.push(
        ...
        '- 2-Fail Rule: triggered per Pre-Action Checkpoint (2 failed attempts → suggest Opus and STOP)',
        ...
        '',
        '### 2-Fail Rule — Enforcement',
        '- What counts as a failure: user reports fix didn\'t work, user rejects the approach, OR a new version published without user confirming previous one worked. "Different approach" does not reset the counter.',
        '- What STOP means: output the suggestion, then write NOTHING — no code, no commands, no publishes, no "but let me also try." Wait for explicit confirmation.',
        '- Each published fix version = one attempt. Bug persists after 2 published versions = rule triggered.',
        '- No self-override: suggesting a switch then continuing anyway is a rule violation. Silence after the suggestion is correct behavior.',
        '',
