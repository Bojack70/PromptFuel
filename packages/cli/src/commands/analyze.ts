...
export async function runAnalyze(promptArg: string | undefined, model: string): Promise<void> {
  ...
  const lines: string[] = [
    ...
    `  Est. output tokens   : ${estimatedOutputTokens.toLocaleString('en-US')}`,
    ...
    `  ${inputTokens.toLocaleString('en-US')} / ${contextWindow.toLocaleString('en-US')} tokens used`,
    `  ${(contextWindow - inputTokens).toLocaleString('en-US')} tokens remaining`,
    '',
    separator('\u2550'),
    '',
  ];
