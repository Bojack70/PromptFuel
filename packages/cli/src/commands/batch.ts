...
export async function runBatch(fileArg: string | undefined, model: string): Promise<void> {
  ...
  const lines: string[] = [
    ...
    '  SUMMARY',
    separator(),
    `  Total input tokens       : ${totalOrigTokens.toLocaleString('en-US')}`,
    `  After optimization       : ${totalOptTokens.toLocaleString('en-US')}`,
    `  Total tokens saved       : ${(totalOrigTokens - totalOptTokens).toLocaleString('en-US')} (${totalOrigTokens > 0 ? Math.round(((totalOrigTokens - totalOptTokens) / totalOrigTokens) * 100) : 0}%)`,
